...
## Yasin-Core SDK Integration (v1.0)

YasinRelay integrates seamlessly with the **Yasin-Core** platform using only the public SDK (`yasin_core.sdk`). This decouples local application orchestration (feed fetching, processing, and publishing pipelines) from the actual AI reasoning, execution, context, and memory runtime layers.

### Flow of AI-Related Execution

```
+---------------------------------------------------------+
|                       YasinRelay                        |
| - Feeds, Parsing, SQLite storage, Telegram/Eitaa pub.   |
| - Initiates processing stages (AIProcessorStage)        |
+----------------------------+----------------------------+
                             |
                             | (Calls _execute_task)
                             v
+----------------------------+----------------------------+
|                    YasinCoreClient (SDK)                |
| - Handles task creation, client context & memory storage|
| - yasin_core.sdk                                        |
+----------------------------+----------------------------+
                             |
                             | (Dispatches to)
...
+----------------------------+----------------------------+
|                         Yasin-Core                        |
| - System-level framework for Agent Lifecycle management|
+----------------------------+----------------------------+
                             |
                             | (Runs inside)
                             v
+----------------------------+----------------------------+
|                       Agent Runtime                     |
| - Executor, Short/Long-Term Memory, Context propagation|
+----------------------------+----------------------------+
                             |
                             | (API Completion calls)
                             v
+----------------------------+----------------------------+
|                         Providers                       |
| - OpenAI / Anthropic / LLMs / Local APIs                |
+---------------------------------------------------------+
```

### Integration Details
1. **YasinCoreClient Initialization**: In `PassthroughProcessor`, `YasinCoreClient` is initialized, providing access to public API features including memory storage, context propagation, and agent registry.
