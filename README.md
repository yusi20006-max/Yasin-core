...
## مدیریت مرکزی پیکربندی (Centralized Configuration Core v1.7)
سیستم مدیریت پیکربندی مرکزی در نسخه v1.7 به هسته Yasin-Core اضافه شده است. این سیستم مسئول بارگذاری، اعتبار سنجی، و ارائه تنظیمات در کل اکوسیستم Yasin است.

### ویژگی‌های کلیدی:
1. **بارگذاری لایه‌ای تنظیمات**: لود تنظیمات به ترتیب اولویت: مقادیر پیش‌فرض (`default.yaml`) <- تنظیمات فایل آدرس‌دهی شده <- متغیرهای محیطی <- تنظیمات اعمال شده در زمان اجرا (Runtime Overrides).
2. **پشتیبانی از متغیرهای محیطی با پیشوند `YASIN_`**: تشخیص و بارگذاری خودکار متغیرهای محیطی و تبدیل هوشمند نوع داده‌ها (مانند تبدیل مقادیر متنی عددی به int/float، مقادیر True/False به Boolean، و آرایه‌ها و دیکشنری‌های JSON به ساختارهای پایتون).
3. **اعتبارسنجی پیشرفته با طرح‌واره (Schema Validation)**: امکان ثبت طرح‌واره برای کلیدها جهت تضمین نوع داده، الزامی بودن فیلدها و اجرای توابع اعتبارسنجی سفارشی (Custom Validator).
4. **پنهان‌سازی مقادیر حساس (Masking Sensitive Values)**: امکان علامت‌گذاری کلیدها به عنوان اطلاعات حساس (مانند توکن‌ها و کلیدهای خصوصی) به طوری که در زمان اجرای متد `status()` مقادیر آن‌ها با `******` پوشانده می‌شود، اما در زمان اجرا به شکل امن از طریق متدها قابل دسترسی است.
5. **یکپارچگی با سیستم پلاگین‌ها**: امکان ثبت پیکربندی‌ها و طرح‌واره‌های اختصاصی پلاگین‌ها تحت فضای نام مجزا با متد `register_plugin_config`.
6. **دسترسی آسان در SDK**: دسترسی مستقیم از طریق `client.config` در `YasinCoreClient` و همچنین از طریق تزریق وابستگی‌ها (DI Container).
...
# خواندن تنظیمات با متدهای کمکی تایپ‌شده
app_name = config.get_string("app.name")
port = config.get_int("app.port", default=8080)
is_debug = config.get_bool("runtime.debug", default=False)

# تعریف طرح‌واره و اعتبارسنجی سفارشی
config.register_schema(
    "app.port",
    int,
    required=True,
    validator=lambda p: 1024 <= p <= 65535,
    description="Ecosystem service port"
)

# اعمال تغییرات موقت در زمان اجرا
